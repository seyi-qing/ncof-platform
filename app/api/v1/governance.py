...
from app.schemas import (
    ...
    CandidateCreate, ElectionPositionCreate, CastVoteIn, DocumentCreate, DocumentUpdate, AnnouncementCreate,
)
...
def get_committee(committee_id: str, db: Session = Depends(get_db), _: User = Depends(require_roles(*MANAGERS, "member"))):
    ...
    return {"id": item.id, "name": item.name, "description": item.description, "status": item.status, "members": members}
...
def get_minutes(meeting_id: str, db: Session = Depends(get_db), _: User = Depends(require_roles(*MANAGERS, "member"))):
    ensure_meeting(db, meeting_id)
    item = db.scalar(select(MeetingMinute).where(MeetingMinute.meeting_id == meeting_id))
    if not item:
...
def list_elections(db: Session = Depends(get_db), _: User = Depends(require_roles(*MANAGERS, "member"))):
    return list(db.scalars(select(Election).order_by(Election.created_at.desc())).all())
...
def update_document(document_id: str, payload: DocumentUpdate, db: Session = Depends(get_db), user: User = Depends(require_roles(*MANAGERS))):
    item = db.get(AssociationDocument, document_id)
    if not item:
...
def list_all_announcements(db: Session = Depends(get_db), _: User = Depends(require_roles(*MANAGERS))):
    return list(db.scalars(select(Announcement).order_by(Announcement.created_at.desc())).all())
...
def publish_announcement(announcement_id: str, db: Session = Depends(get_db), user: User = Depends(require_roles(*MANAGERS))):
    ...
    members = list(db.scalars(select(Member).where(Member.membership_status == "active", Member.user_id.is_not(None))).all())
    db.add_all([Notification(id=str(uuid4()), member_id=m.id, user_id=m.user_id, title=item.title, body=item.body, notification_type="governance", priority="normal") for m in members])
    db.commit()
    db.refresh(item)
    return item
